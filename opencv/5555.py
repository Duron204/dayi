...
def imgLogo():
#1,处理log
#对LOGO进行灰度化处理，得到两个MASK
    img_logo=cv2.imread("./image/loga.png")
    img_target=cv2.imread("./image/cyj.jpg")
    bg_w = img_target.shape[1]
    img_logo = cv2.resize(img_logo, (bg_w//3, int(img_logo.shape[0]*(bg_w//3/img_logo.shape[1]))))
...
#2,处理目标图像的ROI
    ...
    img_roi = img_target[rows1-rows:rows1, cols1- cols:cols1].copy()
...
#3,合并ROI和logo
    ...
    img_target[rows1-rows:rows1, cols1- cols:cols1] = img_res2[:, :]
